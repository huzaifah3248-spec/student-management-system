...
const ADMIN_ROLES = ['admin', 'administrator', 'principal'];
const TERM_1 = 'TERM_1';
...
function validateElectiveSelection(gradeLevel, electiveTrack, electiveSubject) {
  if (gradeLevel >= 1 && gradeLevel <= 8) {
    if (electiveTrack || electiveSubject) {
      return 'Grades 1-8 cannot have elective selections.';
    }
    return null;
  }
...
async function syncStudentEnrollments(conn, studentId, gradeLevel, electiveTrack, electiveSubject, academicYear) {
  const [coreSubjects] = await conn.execute(
    `SELECT id
     FROM subjects
     WHERE (track = 'CORE' OR track = 'BOTH')
       AND min_grade <= ?
       AND max_grade >= ?`,
  ...
  for (const row of coreSubjects) {
    await conn.execute(
      `INSERT IGNORE INTO enrollments (student_id, subject_id, academic_year, grade_level, term)
       VALUES (?, ?, ?, ?, ?)`,
  ...
  if (!electiveRows.length) {
    throw new Error(`Elective subject seed is missing for code ${electiveCode}.`);
  }
...
router.get('/', authenticateJWT, requireRole([...ADMIN_ROLES, 'teacher']), async (req, res, next) => {
  try {
    ...
    let sql = `SELECT s.id, s.student_id, s.user_id, s.first_name, s.last_name,
                       s.date_of_birth, s.grade_level, s.section_no, s.class_roll_no,
                       s.elective_track, s.elective_subject, s.elective_locked,
                       u.username, u.email
                FROM students s
...
// POST /api/students - create student with capacity lock and curriculum assignment
router.post('/', authenticateJWT, requireRole(ADMIN_ROLES), async (req, res, next) => {
  ...
  try {
    ...
    const [classRows] = await conn.execute(
      `SELECT capacity
       FROM classrooms
       WHERE grade_level = ? AND section_no = ?
       FOR UPDATE`,
      [gradeLevel, sectionNo]
    );
    ...
    const capacity = Number(classRows[0].capacity || 35);
    ...
    const [insertResult] = await conn.execute(
      `INSERT INTO students (
        ...
        grade_level, section_no, class_roll_no, elective_track, elective_subject, elective_locked
      )
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        student_id,
    ...
    await syncStudentEnrollments(conn, newStudentId, gradeLevel, track, subject, academicYear);
...
router.get('/me', authenticateJWT, requireRole(['student']), async (req, res, next) => {
  try {
    const [rows] = await pool.execute(
      `SELECT id, student_id, user_id, first_name, last_name, date_of_birth,
              grade_level, section_no, class_roll_no, elective_track, elective_subject, elective_locked
       FROM students
       WHERE user_id = ?
       LIMIT 1`,
...
// PUT /api/students/:id/elective - choose/update Grade 9 electives, or admin override for Grade 10
router.put('/:id/elective', authenticateJWT, async (req, res, next) => {
  ...
  if (!isAdmin && !isStudent) {
    return res.status(403).json({ message: 'Forbidden: only students or admins can update electives.' });
  }
  ...
  try {
    ...
    const [rows] = await conn.execute(
      `SELECT id, user_id, grade_level, elective_locked
       FROM students
       WHERE id = ?
       LIMIT 1
       FOR UPDATE`,
    ...
    await conn.execute(
      `UPDATE students
       SET elective_track = ?, elective_subject = ?, elective_locked = ?
       WHERE id = ?`,
...
// PUT /api/students/:id/promote - promote to next grade with locked capacity
router.put('/:id/promote', authenticateJWT, requireRole(ADMIN_ROLES), async (req, res, next) => {
  ...
  try {
    ...
    const [studentRows] = await conn.execute(
      `SELECT id, grade_level, section_no, class_roll_no, elective_track, elective_subject
       FROM students
       WHERE id = ?
       LIMIT 1
       FOR UPDATE`,
    ...
    if (Number.isNaN(targetSection) || Number.isNaN(targetRollNo)) {
      await conn.rollback();
      return res.status(400).json({ message: 'section_no and class_roll_no must be numeric when provided.' });
    }
    ...
    const [countRows] = await conn.execute(
      `SELECT COUNT(*) AS cnt
       FROM students
       WHERE grade_level = ? AND section_no = ? AND id <> ?
       FOR UPDATE`,
      [nextGrade, targetSection, studentId]
    );
    ...
    await conn.execute(
      `UPDATE students
       SET grade_level = ?, section_no = ?, class_roll_no = ?, elective_track = ?, elective_subject = ?, elective_locked = ?
       WHERE id = ?`,
